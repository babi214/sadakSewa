...
function getStatusColor(status) {
  return STATUS_MARKER_COLORS[status?.toLowerCase()] || '#6F777D'
}
...
export function createReportMarkerIcon(isActive = false, status) {
  const color = getStatusColor(status)
  return L.divIcon({
    className: '',
    html: `
      <div class="report-map-marker ${isActive ? 'report-map-marker--active' : ''}">
        <div class="report-map-marker__shield" style="background: ${color}">
          ${CONSTRUCTION_SVG}
        </div>
